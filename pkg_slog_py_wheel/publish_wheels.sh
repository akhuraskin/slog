...
Version is read automatically from the VERSION file (currently: ${VERSION}).
...
Example resulting paths (dev):
  ${ARTIFACTORY_BASE}/${REPO_DEV_PATH}/slog-py/<version>/slog_py-<version>-cp38-cp38-manylinux2014_x86_64.whl
  ${ARTIFACTORY_BASE}/${REPO_DEV_PATH}/slog-py/<version>/slog_py-<version>-cp39-cp39-manylinux2014_x86_64.whl
  ${ARTIFACTORY_BASE}/${REPO_DEV_PATH}/slog-py/<version>/slog_py-<version>-cp310-cp310-manylinux2014_x86_64.whl

Example resulting paths (--prod):
  ${ARTIFACTORY_BASE}/${REPO_PROD_PATH}/slog-py/<version>/slog_py-<version>-cp38-cp38-manylinux2014_x86_64.whl
  ${ARTIFACTORY_BASE}/${REPO_PROD_PATH}/slog-py/<version>/slog_py-<version>-cp39-cp39-manylinux2014_x86_64.whl
  ${ARTIFACTORY_BASE}/${REPO_PROD_PATH}/slog-py/<version>/slog_py-<version>-cp310-cp310-manylinux2014_x86_64.whl
EOF
}
...
ARTIFACTORY_USER="${ARTIFACTORY_USER:-$(python3 -c "import netrc; print(netrc.netrc().authenticators('na1-artifactory.stargate.toyota')[0])" 2>/dev/null)}"
ARTIFACTORY_TOKEN="${ARTIFACTORY_TOKEN:-$(python3 -c "import netrc; print(netrc.netrc().authenticators('na1-artifactory.stargate.toyota')[2])" 2>/dev/null)}"
